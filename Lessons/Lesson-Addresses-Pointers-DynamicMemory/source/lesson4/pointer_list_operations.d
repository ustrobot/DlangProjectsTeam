 ...
 * Example: Pointer-Based List Operations
 *
 * This example demonstrates common operations on linked lists using pointer-based nodes.
 */
module lesson4.pointer_list_operations;
...
void runExample() {
    writeln("=== Pointer-Based List Operations Example ===\n");
    ...
    writeln("\nKey takeaways:");
    writeln("1. List operations with pointers require careful memory management");
    writeln("2. Many algorithms use the two-pointer technique (fast/slow pointers)");
    writeln("3. Pointer-based operations give direct control over memory");
    writeln("4. Detecting cycles is important for preventing infinite loops");
    writeln("5. Manual memory management requires explicit deallocation to prevent leaks");
}
...
void removeDuplicates(LinkedList!int list) {
    ...
    while (current !is null && current.next !is null) {
        if (current.value == current.next.value) {
            // Duplicate found, store the node to be removed
