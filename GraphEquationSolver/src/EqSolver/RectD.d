...
struct RectD {
    ...
    @property double middlex() const { return (left + right) / 2; }
    /// returns average of top, bottom
    @property double middley() const { return (top + bottom) / 2; }
    /// returns middle point
    @property PointD middle() const { return PointD(middlex, middley); }

    /// returns top left point of rectangle
    @property PointD topLeft() const { return PointD(left, top); }
    /// returns bottom right point of rectangle
    @property PointD bottomRight() const { return PointD(right, bottom); }
    /// returns top right point of rectangel
    @property PointD topRight() const { return PointD(right, top); }
    /// returns bottom left point of rectangle
    @property PointD bottomLeft() const { return PointD(left, bottom); }

    /// returns size (width, height) in Point
    @property PointD size() const { return PointD(right - left, bottom - top); }
    
        /// returns width of rectangle (right - left)
    @property double width() const { return right - left; }
    /// returns height of rectangle (bottom - top)
    @property double height() const { return bottom - top; }
